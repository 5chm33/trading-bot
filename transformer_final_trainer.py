...
# Configure the root logger at the start of the script
logging.basicConfig(
    level=logging.DEBUG,  # Set the base logging level
    format='%(asctime)s | %(levelname)s | %(message)s',  # Log format
    handlers=[
        logging.FileHandler('trading_bot.log', encoding='utf-8'),  # Log to file
...
def combine_data(all_data, config):
    ...
    logging.debug(f"Combined data before flattening:\n{combined_data.head()}")
    logging.debug(f"Combined data columns before flattening:\n{combined_data.columns}")

    # Flatten MultiIndex columns
    combined_data = flatten_columns(combined_data)

    # Log the combined data after flattening
    logging.debug(f"Combined data after flattening:\n{combined_data.head()}")
    logging.debug(f"Combined data columns after flattening:\n{combined_data.columns}")

    # Remove duplicated columns
    combined_data = remove_duplicated_columns(combined_data)

    # Log the combined data after removing duplicates
    logging.debug(f"Combined data after removing duplicates:\n{combined_data.head()}")
    logging.debug(f"Combined data columns after removing duplicates:\n{combined_data.columns}")
    ...
    for ticker in config['tickers']:
        logging.info(f"Adding technical indicators for {ticker}.")
        combined_data = trainer.add_technical_indicators(combined_data, ticker)

    # Log the combined data after adding indicators
    logging.debug(f"Combined data after adding indicators:\n{combined_data.head()}")
    logging.debug(f"Combined data columns after adding indicators:\n{combined_data.columns}")

    # Handle missing data
    combined_data = trainer.handle_missing_data(combined_data)

    # Log the combined data after handling missing data
    logging.debug(f"Combined data after handling missing data:\n{combined_data.head()}")
    logging.debug(f"Combined data columns after handling missing data:\n{combined_data.columns}")

    # Drop columns with all NaN values
    combined_data = combined_data.dropna(axis=1, how='all')

    # Log the DataFrame shape and columns after dropping NaN columns
    logging.debug(f"DataFrame shape after dropping NaN columns: {combined_data.shape}")
    logging.debug(f"Columns after dropping NaN columns:\n{combined_data.columns}")

    # Check if DataFrame is empty
    if combined_data.empty:
        logging.error("Combined DataFrame is empty. Check the input data.")
        return combined_data

    # Log the final DataFrame
    logging.debug(f"Final DataFrame:\n{combined_data}")
    logging.debug(f"DataFrame columns:\n{combined_data.columns}")
...
class TransformerTrainer:
    ...
    def train_with_ray_tune(self, config, checkpoint_dir=None):
        """Train the model with Ray Tune."""
        try:
            # Prepare data
            logger.info("Preparing data...")
            prepared_data = self.prepare_data(ticker='AAPL')  # Replace with your ticker
    ...
    def prepare_data(self, ticker):
        try:
            ...
            # Step 2: Calculate target variable (percentage change in price)
            ...
            # Step 3: Drop rows with NaN in the target
            ...
            # Step 5: Log feature names
            self.feature_names = features.columns.tolist()
            self.logger.debug(f"Feature names: {self.feature_names}")

            # Step 6: Scale the features and target
            self.logger.info("Scaling features and target using MinMaxScaler.")
            self.scaler = MinMaxScaler(feature_range=(0, 1))  # Ensure MinMaxScaler is used here
            ...
            self.target_scaler = MinMaxScaler(feature_range=(0, 1))  # Ensure MinMaxScaler is used here
            ...
            # Step 7: Create sequences for Transformer
            ...
            # Step 8: Split into training, validation, and test sets
            ...
            # Step 9: Check for NaN or Inf in the data
    ...
    def build_transformer_model(self, input_shape, num_features, num_heads=4, dropout_rate=0.3, num_layers=2, ff_dim=256, l2_reg=1e-5):
    ...
    def build_model(self, hyperparameters=None):
        try:
            # Use best hyperparameters if none are provided
            if hyperparameters is None:
                hyperparameters = {
                    "learning_rate": 4.238547370643523e-06,
                    "num_heads": 4,
                    "dropout_rate": 0.14280463270279206,
                    "num_layers": 4,
                    "ff_dim": 256,
                    "batch_size": 32,
                    "l2_reg": 5.530952225965722e-06
                }

            # Extract hyperparameters
            learning_rate = hyperparameters.get('learning_rate', 1.4496173314481278e-05)
            num_heads = hyperparameters.get('num_heads', 4)
            dropout_rate = hyperparameters.get('dropout_rate', 0.4621532335780453)
            num_layers = hyperparameters.get('num_layers', 3)
            ff_dim = hyperparameters.get('ff_dim', 512)
            l2_reg = hyperparameters.get('l2_reg', 2.9372259481154597e-05)
    ...
    def train_final_model(self, best_params, epochs=100, batch_size=32):
        try:
            ...
            # Step 1: Prepare data
            if not hasattr(self, '_prepared_data') or self._prepared_data is None:
                self.logger.info("Preparing data for final model training.")
            ...
            self.logger.info("Building the model with best hyperparameters.")
            model = self.build_model(best_params)
            if model is None:
                self.logger.error("Model building failed. Cannot proceed with training.")
                return None, None

            # Step 3: Log the model summary
            self.logger.info("Model summary:")
            model.summary(print_fn=lambda x: self.logger.info(x))

            # Step 4: Train the model
            self.logger.info("Training the model with best hyperparameters.")
...
def main():
    try:
        ...
        # **NEW**: Debugging block to test prepare_data
        logger.info("Testing prepare_data method.")
        ...
        final_model, final_history = trainer.train_final_model(best_hyperparams, epochs=2000)

        if final_model is not None:
            # Save the final model
            logger.info("Saving the final model and scalers.")
            try:
                final_model.save('final_trading_model.keras')
                logger.info("Final model saved to 'final_trading_model.keras'.")
            except Exception as e:
                logger.error(f"Failed to save the final model: {e}", exc_info=True)

            # Save the scalers
            try:
                joblib.dump(trainer.scaler, 'final_scaler.pkl')
                joblib.dump(trainer.target_scaler, 'final_target_scaler.pkl')
                logger.info("Scalers saved to 'final_scaler.pkl' and 'final_target_scaler.pkl'.")
            except Exception as e:
                logger.error(f"Failed to save scalers: {e}", exc_info=True)
        else:
            logger.error("Failed to train the final model. Check logs for errors.")
...
def test_prepare_data():
    ...
    try:
        logger.info("Testing data preparation process.")
        ...
        logger.error(f"Error in test_prepare_data: {e}", exc_info=True)

if __name__ == "__main__":
    # Ensure the logger is configured
    if not logger.handlers:
        logger.debug("Starting the trading bot.")
